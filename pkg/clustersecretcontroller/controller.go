package clustersecretcontroller
...
type Controller struct {
	...
	lister    lister.ClusterSecretLister
	wq        workqueue.RateLimitingInterface
}

func NewController(clientset clientset.Interface, clusterSecretInformer inf.ClusterSecretInformer) *Controller {
	controller := &Controller{
		clientset: clientset,
		synced:    clusterSecretInformer.Informer().HasSynced,
		lister:    clusterSecretInformer.Lister(),
		wq:        workqueue.NewRateLimitingQueue(workqueue.DefaultControllerRateLimiter()),
	}

	clusterSecretInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
...
func (controller *Controller) handleAdd(obj interface{}) {
	...
	if err != nil {
		log.Printf("cluster-secret controller: error getting key for %s: %s\n", obj, err.Error())
...
func (controller *Controller) Run(ch chan struct{}) error {
	if ok := cache.WaitForCacheSync(ch, controller.synced); !ok {
		return errors.New("cluster-secret controller: failed to sync informer cache for ClusterSecret controller")
	}
...
func (controller *Controller) processNextItem() bool {
	...
	if err := controller.processItem(key.(string)); err != nil {
		log.Printf("cluster-secret controller: error processing %s: %s\n", key, err.Error())
		log.Printf("cluster-secret controller: %s will be requeued\n", key)
...
func (controller *Controller) processItem(key string) error {
	log.Printf("cluster-secret controller: key: %s\n", key)
